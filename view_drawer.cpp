...
void ViewDrawer::draw_screen() {
	...
	//Set the view matrix to just the player's orientation
	...
	//Update the view matrix with the player's position
	glTranslatef(-game.get_logic().get_player_loc(0), -game.get_logic().get_player_loc(2), game.get_logic().get_player_loc(1));
	
	// glMatrixMode(GL_PROJECTION);
	// glPushMatrix();
	// glLoadIdentity();
	// glOrtho(-10, 10, -10, 10, .001, 1000);
