...
array<float, 2> PerlinGrid::point_gradient(int x, int y) {
	return array<float, 2>({{
		(float)pointhash(x, y, seed)/(float)(hvt)(-1)*2-1,
		(float)pointhash(x, y, seed ^ (uint64_t)1<<63)/(float)(hvt)(-1)*2-1
	}});
}
...
float PerlinGrid::grid_value(float x, float y, float px, float py) {
	array<float, 2> grid_grad = point_gradient(px, py);
